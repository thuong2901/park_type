...
const mapStateToProps = state => {
	return {
		history_parks: state.history_parks,
	}
}

const mapDispatchToProps = dispatch => ({
	fetchHistoryParks: () => dispatch(fetchHistoryParks()),
  postDeleteHistorypark: (parks) => dispatch(postDeleteHistorypark(parks))
});
...
const EnhancedTableToolbar = (props) => {
  ...
  const handleDelete = async () => {
      ...
      var response = await props.postDeleteHistorypark(selected);
      if (response) {
...
function HistoryPark(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds =  [];
      const parks = props.history_parks.parks;
      for (let i = 0; i < parks.length; ++i) {
        if (parks[i].status != 'Đang đỗ xe') {
          newSelecteds.push(parks[i].parking_id);
        }
      }
      //props.history_parks.parks.map((n) => n.parking_id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - props.history_parks.parks.length) : 0;
            ...
            <Box sx={{ width: '100%' }}>
              <Paper sx={{ width: '100%'}}>
                ...
                <TableContainer>
                  ...
                  >
                    <EnhancedTableHead
                      ...
                      rowCount={props.history_parks.parks.length}
                    />
                    <TableBody>
                      {/* if you don't need to support IE11, you can replace the `stableSort` call with:
                        rows.slice().sort(getComparator(order, orderBy)) */}
                      {stableSort(props.history_parks.parks, getComparator(order, orderBy))
                        .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                        .map((row, index) => {
                          const isItemSelected = isSelected(row.parking_id);
                ...
                <TablePagination
                  ...
                  component="div"
                  count={props.history_parks.parks.length}
...
export default connect(mapStateToProps, mapDispatchToProps)(HistoryPark);